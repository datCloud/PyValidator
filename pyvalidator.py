...
try:
    ...
    def W3CValidation(url, sitemap = False, reportPath = False):
        reportPath = '.' if not reportPath else reportPath
        os.system(f'htmlcheck -v 0 page {url} --exporter json --destination {reportPath}')
    ...
    def CheckValidationLog(url):
        ...
        for key, value in jsonLog['statistics'].items():
            if key == 'debugs' or int(value) == 0: continue
            if not hasErrors:
                hasErrors = True
